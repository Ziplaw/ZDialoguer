...
namespace ZDialoguer
{
    public class DialogueBlackboard : Blackboard
    {
        ...
        public DialogueBlackboard(ZDialoguerGraphView graphView) : base(graphView)
        {
            ...
            schedule.Execute(() =>
            {
                SetPosition(new Rect(
                    ...
                    new Vector2(230, 345)));
            }).Until(() => !float.IsNaN(resolvedStyle.width)  );
        }
        ...
        {
            graphView.graph.facts.ForEach(f => { this.Query<BlackboardSection>().ToList().First(s => s.title == "Facts").Add(GenerateFactContainer(f)); });
        }
